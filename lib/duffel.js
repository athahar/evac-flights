...
export async function duffelFetch(config, path, options = {}) {
...
export async function searchOffersPersonal(config, params) {
  const {
  ...
  const passengers = [
    ...
    ...Array.from({ length: children }, () => ({ type: "child", age: 10 }))
  ];
  ...
  const createPayload = { data: { slices, passengers, cabin_class: cabinClass } };
  duffelLog(config, `personal search ${origin}->${destination} depart=${departureDate} return=${returnDate || "none"} adults=${adults} children=${children} cabin=${cabinClass}`);
  ...
  if (!requestId) throw new Error("Duffel offer request returned no request id");

  try {
    const offersResult = await duffelFetch(config, `/air/offers?offer_request_id=${encodeURIComponent(requestId)}&limit=200`);
    const offers = Array.isArray(offersResult?.data) ? offersResult.data : [];
    duffelLog(config, `personal search result requestId=${requestId} offers=${offers.length}`);
    if (offers.length > 0) return { requestId, offers };
  ...
  duffelLog(config, `personal search result requestId=${requestId} offers=${embeddedOffers.length} source=embedded`);
