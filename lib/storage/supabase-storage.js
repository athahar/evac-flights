...
function buildDestinationCityMap(fr24InputFile) {
  const map = new Map();
  if (!fr24InputFile) return map;

  try {
    const rows = parseFr24File(fr24InputFile);
    ...
    console.warn(`[supabase] could not build destination city map from FR24 input: ${err.message}`);
  }
...
export function createSupabaseStorage(supabase, config = {}) {
  const destinationCityMap = buildDestinationCityMap(config.fr24InputFile);
  ...
  return {
    ...
    async getActiveSnapshot() {
      ...
      return {
        ...
        rows: (flights || []).map((row) => fromFlightRow(row, destinationCityMap))
      };
    },
