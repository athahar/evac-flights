  ...
  return {
    ...
    async saveSearchQuery(payload) {
      const insertPayload = {
        ...
        query_signature: payload.querySignature,
        origin: payload.origin,
        departure_date: payload.departureDate,
        destinations_json: payload.destinations || [],
        destinations_blocked_json: payload.destinationsBlocked || [],
        source: payload.source || "live",
        pairs_checked: payload.pairsChecked || 0,
        offers_found: payload.offersFound || 0,
      ...
      if (error) throw new Error(`saveSearchQuery failed: ${error.message}`);
      return data?.id || "";
    },

    async getRecentSearchBySignature(querySignature, maxAgeSeconds) {
      const cutoffIso = new Date(Date.now() - Math.max(0, maxAgeSeconds) * 1000).toISOString();
      ...
      if (error) throw new Error(`getRecentSearchBySignature failed: ${error.message}`);
      return normalizeSearchQueryRow(data);
    },
