...
function buildScrapeFailureHtml(reason, airports, fallbackRowCount) {
  const ts = new Date().toISOString();
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.5;">
      <h2 style="color:#c0392b;">⚠️ FR24 Scrape Source Failed</h2>
      <p>The FR24 free schedule API scrape failed and the dashboard fell back to file-based data.</p>
      <table style="border-collapse:collapse;">
        <tr><td style="padding:6px 12px;font-weight:bold;">Reason</td><td style="padding:6px 12px;">${escapeHtml(reason)}</td></tr>
        <tr><td style="padding:6px 12px;font-weight:bold;">Airports</td><td style="padding:6px 12px;">${escapeHtml(airports.join(", "))}</td></tr>
        <tr><td style="padding:6px 12px;font-weight:bold;">Fallback rows</td><td style="padding:6px 12px;">${fallbackRowCount}</td></tr>
        <tr><td style="padding:6px 12px;font-weight:bold;">Timestamp</td><td style="padding:6px 12px;">${escapeHtml(ts)}</td></tr>
      </table>
      <p style="margin-top:16px;">If this persists, the free FR24 API may be down or rate-limited. Consider re-enabling the paid API or refreshing manual data files.</p>
      <p style="color:#666;font-size:12px;">Automated alert from evac-flight-alert scrape monitor.</p>
    </div>
  `;
}

export async function sendScrapeFailureEmail(config, reason, airports, fallbackRowCount) {
  if (!config.resendApiKey || !config.alertEmailFrom || !config.alertEmailTo) {
    console.warn("[email] scrape failure notification skipped — email not configured");
    return null;
  }

  const subject = `⚠️ FR24 scrape failed: ${reason.slice(0, 80)}`;
  const text = [
    "FR24 Scrape Source Failed",
    "",
    `Reason: ${reason}`,
    `Airports: ${airports.join(", ")}`,
    `Fallback rows loaded: ${fallbackRowCount}`,
    `Timestamp: ${new Date().toISOString()}`,
    "",
    "The dashboard fell back to file-based data.",
    "If this persists, the free FR24 API may be down or rate-limited."
  ].join("\n");
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject,
        text,
        html: buildScrapeFailureHtml(reason, airports, fallbackRowCount)
      })
    });

    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.error(`[email] scrape failure notification failed: ${payload?.message || response.status}`);
      return null;
    }
    console.log("[email] scrape failure notification sent");
    return payload;
  } catch (err) {
    // Don't let notification failure break the pipeline
    console.error(`[email] scrape failure notification error: ${err.message}`);
    return null;
  }
}
