...
import { sendScrapeFailureEmail } from "./email.js";
...
export function normalizeStatus(rawStatus) {
  ...
  if (lower.includes("estim") || lower.includes("delay")) return "Estimated";
...
// ── FR24 Internal Schedule API ("scrape") normalizer ──────────────────────
// This is a SEPARATE normalizer from normalizeApiFlight() because the free
// internal API uses different JSON paths (item.flight.time.scheduled.departure
// vs item.departure.scheduled, etc.).

const FR24_SCRAPE_BASE_URL =
  "https://api.flightradar24.com/common/v1/airport.json";

export function normalizeScrapeApiFlight(item, airportCode) {
  const f = item?.flight;
  if (!f) return null;

  // ── Scheduled departure (required) ──
  const scheduledDeparture = f?.time?.scheduled?.departure;
  if (!scheduledDeparture) return null;

  // ── Timezone from origin airport (no hardcoded map) ──
  const timezone = f?.airport?.origin?.timezone?.name || "UTC";
  const departureDate = new Date(scheduledDeparture * 1000);
  const local = buildLocalDateParts(departureDate, timezone);

  // ── Flight code ──
  const flightCode = String(f?.identification?.number?.default || "")
    .trim()
    .toUpperCase();

  // ── Carrier code: prefer airline.code.iata, fallback to parsing flight code ──
  const directCarrier = String(f?.airline?.code?.iata || "")
    .trim()
    .toUpperCase();
  const parsed = parseFlightCode(flightCode);
  const carrierCode = directCarrier || parsed.carrierCode;
  ...
  const destinationCity = String(
    f?.airport?.destination?.position?.region?.city || ""
  ).trim();

  // ── Airline ──
  const airline = String(f?.airline?.name || "").trim();

  // ── Aircraft: model code + optional registration ──
  const modelCode = String(f?.aircraft?.model?.code || "").trim();
  const registration = String(f?.aircraft?.registration || "").trim();
  const aircraft = registration ? `${modelCode} (${registration})` : modelCode;

  // ── Status ──
  const statusRaw = String(
    f?.status?.generic?.status?.text || ""
  ).trim();
  const status = normalizeStatus(statusRaw || "Scheduled");

  // ── Estimated departure: time.estimated.departure OR status.generic.eventTime.utc ──
  const estimatedDeparture =
    f?.time?.estimated?.departure ||
    f?.status?.generic?.eventTime?.utc ||
    null;

  let estimatedDepartureLocalIso = null;
  if (estimatedDeparture) {
    const estDate = new Date(estimatedDeparture * 1000);
    if (!Number.isNaN(estDate.getTime())) {
      estimatedDepartureLocalIso = buildLocalDateParts(estDate, timezone).localIso;
    }
  }

  return {
    originAirport: String(airportCode || "").trim().toUpperCase(),
    flightDate: local.dateIso,
    timeLocal: local.timeLocal,
    departureLocalIso: local.localIso,
    flight: flightCode,
    ...
    scheduledDeparture,
    estimatedDeparture,
    estimatedDepartureLocalIso,
    timezone
  };
}

async function fetchFr24ScrapePage(airportCode, page, timestamp, timeoutMs) {
...
async function fetchFr24ScrapeRows(config) {
  const pageDelayMs = Number.isInteger(config?.fr24ScrapePageDelayMs)
    ? config.fr24ScrapePageDelayMs
    : 500;
  const timeoutMs = Number.isInteger(config?.fr24ApiTimeoutMs)
    ? config.fr24ApiTimeoutMs
    : 15000;
  ...
  for (const airport of config.originAirports) {
    const airportUpper = String(airport || "").trim().toUpperCase();
    const timestamp = Math.floor(Date.now() / 1000);
    let page = 1;
    let totalPages = 1;
    let airportFlights = 0;

    while (page <= totalPages) {
      const data = await fetchFr24ScrapePage(
        airportUpper,
        page,
        timestamp,
        timeoutMs
      ...
      if (!schedule) {
        console.warn(
          `[fr24] scrape: no schedule data for ${airportUpper} page ${page}`
        );
        break;
      }
      ...
      airportFlights += flights.length;

      for (const item of flights) {
        const normalized = normalizeScrapeApiFlight(item, airportUpper);
        if (normalized) {
      ...
      console.log(
        `[fr24] scrape: ${airportUpper} page ${page}/${totalPages} flights=${flights.length}`
      );
    ...
    perAirport[airportUpper] = airportFlights;
  }
  ...
  if (!validation.valid) {
    ...
    throw new Error(`FR24 scrape data failed validation: ${reasons}`);
  }
...
function loadConfiguredFr24FileRows(config) {
  ...
  const missingOrigins = [...configuredOrigins].filter(
    (o) => !selectedEntries.some(([origin]) => String(origin).trim().toUpperCase() === o)
  );
  if (missingOrigins.length > 0) {
    console.warn(`[fr24] configured origins missing from FR24_INPUT_FILES_BY_ORIGIN: ${missingOrigins.join(", ")}`);
  }
  ...
  for (const [origin, filePath] of selectedEntries) {
    ...
    console.log(`[fr24] loaded origin=${origin} rows=${fileRows.length} file=${filePath}`);
    rows.push(...fileRows);
...
export async function loadFr24Data(config, options = {}) {
  ...
  // ── Scrape source (free internal API, behind feature flag) ──
  if (config?.fr24ScrapeEnabled) {
    try {
      const scrapeResult = await fetchFr24ScrapeRows(config);
      const scrapeRows = scrapeResult.rows || [];
      const scrapeMeta = scrapeResult.meta || { source: "fr24-scrape" };
      const { validation = {} } = scrapeMeta;
      console.log(
        `[fr24] source=fr24-scrape rows=${scrapeRows.length} raw=${scrapeMeta.rawRowsCount || 0} airports=${Object.keys(scrapeMeta.perAirport || {}).join(",")} statusCoverage=${(validation.coverage?.statusCoverage || 0).toFixed(2)}`
      );
      if (scrapeRows.length > 0 || !config.fr24ScrapeFallbackToFile) {
        return {
          rows: scrapeRows,
          meta: scrapeMeta
        };
      }
      fallbackReason = "scrape_empty_rows";
      console.warn("[fr24] scrape returned no rows, falling back to file source");
      const { rows: fileRows, filePaths } = loadConfiguredFr24FileRows(config);
      sendScrapeFailureEmail(config, fallbackReason, config.originAirports, fileRows.length).catch(() => {});
      return {
        rows: fileRows,
        meta: {
          source: "file",
      ...
      console.error(`[fr24] scrape fetch failed: ${err.message}`);
      if (!config.fr24ScrapeFallbackToFile) {
        throw err;
      }
