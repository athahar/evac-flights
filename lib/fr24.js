...
function extractFlightsArray(payload, responsePath) {
...
function normalizeApiFlight(item, timezone) {
...
async function fetchFr24ApiRows(config, allowedDates) {
  ...
  const authHeader = String(config.fr24ApiAuthHeader || "x-apikey").trim();
...
export async function loadFr24Data(config, options = {}) {
  ...
  const fileRows = parseFr24File(config.fr24InputFile);
  console.log(`[fr24] source=file rows=${fileRows.length} path=${config.fr24InputFile}`);
  return {
    rows: fileRows,
    meta: {
      source: "file",
