...
function parseLookaheadByOrigin(value) {
  const out = {};
  const raw = String(value || "").trim();
  const text = raw.startsWith("=") ? raw.slice(1) : raw;
  if (!text) return out;
  ...
  for (const token of text.split(",")) {
    ...
    const origin = pair.slice(0, eqIndex).trim().toUpperCase();
    const days = parseIntStrict(pair.slice(eqIndex + 1).trim(), 0);
    if (!origin || days <= 0) {
      skipped.push(pair);
      continue;
    }
  ...
  if (skipped.length > 0) {
    console.warn(`[config] DASHBOARD_LOOKAHEAD_DAYS_BY_ORIGIN: skipped malformed tokens: ${skipped.join(", ")}`);
  }
...
export function loadConfig(env) {
  ...
  const searchAirportsFile = resolveExistingPath(cwd, env.SEARCH_AIRPORTS_FILE, [
    "./public/data/airports.curated.json"
  ]);
  ...
  return {
    ...
    fr24ScrapeEnabled: parseBool(env.FR24_SCRAPE_ENABLED, false),
    fr24ScrapePageDelayMs: parseIntStrict(env.FR24_SCRAPE_PAGE_DELAY_MS, 500),
    fr24ScrapeFallbackToFile: parseBool(env.FR24_SCRAPE_FALLBACK_TO_FILE, true),
    airlinesFile,
    ...
    dashboardLookaheadDaysByOrigin: parseLookaheadByOrigin(env.DASHBOARD_LOOKAHEAD_DAYS_BY_ORIGIN),
    ...
    searchEnabled: parseBool(env.SEARCH_ENABLED, true),
    searchMaxDestinations: Math.max(1, parseIntStrict(env.SEARCH_MAX_DESTINATIONS, 3)),
    searchCooldownSeconds: Math.max(1, parseIntStrict(env.SEARCH_COOLDOWN_SECONDS, 30)),
    searchCacheTtlSeconds: Math.max(1, parseIntStrict(env.SEARCH_CACHE_TTL_SECONDS, 300)),
    searchDateRangeDays: Math.max(1, parseIntStrict(env.SEARCH_DATE_RANGE_DAYS, 7)),
    searchGlobalMaxLivePerHour: Math.max(1, parseIntStrict(env.SEARCH_GLOBAL_MAX_LIVE_PER_HOUR, 30)),
    searchAllowedOrigins: (env.SEARCH_ALLOWED_ORIGINS || "")
      .split(",")
      .map((s) => s.trim().toUpperCase())
      .filter(Boolean),
    searchAirportsFile,
...
export function assertRequiredConfig(config) {
  // Fall back searchAllowedOrigins to originAirports if env var was not set
  if (config.searchAllowedOrigins.length === 0) {
    config.searchAllowedOrigins = [...config.originAirports];
  }
  ...
  if (config.searchEnabled && !fs.existsSync(config.searchAirportsFile)) {
    missing.push(`SEARCH_AIRPORTS_FILE (resolved: ${config.searchAirportsFile})`);
  }
