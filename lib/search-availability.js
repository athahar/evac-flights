...
async function searchDestination({ config, origin, destinationIata, departureDate, airportsByIata }) {
  ...
  return sortAndTopOffers(mapped, 6);
}
  ...
  async function searchAvailability(input, options = {}) {
    ...
    const destinationSearches = await Promise.all(
      split.checked.map(async (destinationIata) => {
        try {
          const flights = await searchDestination({ config, origin, destinationIata, departureDate, airportsByIata });
