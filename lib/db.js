...
export function createDb(dbPath) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS search_queries (
      ...
      destinations_blocked_json TEXT NOT NULL DEFAULT '[]',
...
export function insertSearchQuery(db, payload) {
  const stmt = db.prepare(`
  ...
  const result = stmt.run(
    ...
    payload.querySignature,
    payload.origin,
    payload.departureDate,
    payload.destinationsJson,
    payload.destinationsBlockedJson || "[]",
    payload.source || "live",
    payload.pairsChecked || 0,
    payload.offersFound || 0,
...
export function getLatestSearchQueryBySignature(db, querySignature) {
  const stmt = db.prepare(`
    SELECT *
    FROM search_queries
    ...
    LIMIT 1
  `);
  return stmt.get(querySignature) || null;
}
