...
export function createDashboardRunner({ storage, config }) {
  ...
  async function runNow(reason = "manual") {
    ...
    try {
      ...
      console.log(
        `[dashboard] fr24 source=${fr24Meta.source || "unknown"} raw=${fr24Meta.rawRowsCount || 0} normalized=${fr24Meta.normalizedRowsCount || parsedRows.length} filtered=${filteredRows.length} requests=${fr24Metrics.requests || 0} retries=${fr24Metrics.retries || 0}${fr24Meta.fallbackReason ? ` fallback=${fr24Meta.fallbackReason}` : ""}`
      );

      const defaultOrigin = config.originAirports[0];
