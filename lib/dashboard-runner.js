...
import { filterFr24Rows, getDateRangeIso, getFr24CreditUsage, loadFr24Data } from "./fr24.js";
...
function rowId(row) {
  return [row.originAirport || "", row.flightDate, row.flight, row.destinationIata, row.timeLocal].join("|");
}
...
/**
 * Returns true if current Dubai time is within a grace window right after
 * an aligned slot boundary (e.g. 05:00:00-05:01:59 for 30-min interval).
 * This lets us catch up a just-missed slot after process restarts.
 */
function isWithinAlignedSlotGrace(intervalMinutes, graceMs = 2 * 60 * 1000) {
  const DUBAI_OFFSET_MS = 4 * 60 * 60 * 1000;
  const intervalMs = Math.max(1, intervalMinutes) * 60 * 1000;
  const nowDubai = Date.now() + DUBAI_OFFSET_MS;
  const msIntoSlot = nowDubai % intervalMs;
  return msIntoSlot >= 0 && msIntoSlot < graceMs;
}
...
function buildTasks(rows) {
  ...
  rows.forEach((row, index) => {
    const key = `${row.origin}|${row.flightDate}|${row.destinationIata}`;
    if (!map.has(key)) {
      map.set(key, {
        key,
        origin: row.origin,
...
export function createDashboardRunner({ storage, config }) {
  ...
  async function runNow(reason = "manual", runOptions = {}) {
    if (runInFlight) {
    ...
    runInFlight = true;
    try {
      const configuredOrigins = (config.originAirports || []).map((x) => String(x || "").trim().toUpperCase()).filter(Boolean);
      const configuredOriginSet = new Set(configuredOrigins);
      const requestedOrigins = (runOptions?.origins || [])
        .map((x) => String(x || "").trim().toUpperCase())
        .filter(Boolean)
        .filter((origin, index, arr) => arr.indexOf(origin) === index)
        .filter((origin) => configuredOriginSet.has(origin));
      const runOrigins = requestedOrigins.length > 0 ? requestedOrigins : configuredOrigins;
      ...
      const runConfig = {
        ...config,
        originAirports: runOrigins
      };
      const defaultOrigin = runOrigins[0] || config.originAirports[0];

      // Per-origin lookahead: compute date windows per origin, use max union for loading
      const lookaheadMap = runConfig.dashboardLookaheadDaysByOrigin || {};
      const perOriginDates = {};
      let maxLookahead = runConfig.dashboardLookaheadDays;
      for (const origin of runOrigins) {
        const days = lookaheadMap[origin] || runConfig.dashboardLookaheadDays;
        perOriginDates[origin] = new Set(getDateRangeIso(runConfig.dashboardTimezone, days));
        if (days > maxLookahead) maxLookahead = days;
      }
      const allowedDates = getDateRangeIso(runConfig.dashboardTimezone, maxLookahead);
      console.log(`[dashboard] lookahead windows: ${runOrigins.map((o) => `${o}=${lookaheadMap[o] || runConfig.dashboardLookaheadDays}d`).join(" ")}`);

      const fr24LoadStartedAtMs = Date.now();
      const fr24Loaded = await loadFr24Data(runConfig, { allowedDates });
      const fr24LoadLatencyMs = Date.now() - fr24LoadStartedAtMs;
      const parsedRows = fr24Loaded.rows || [];
      const filteredRows = filterFr24Rows(parsedRows, {
        blockedAirports: runConfig.blocklist?.airports || [],
        allowedStatuses: ["Scheduled", "Estimated"],
        allowedDates
      });
      const runOriginSet = new Set(runOrigins);
      const scopedRows = filteredRows.filter((row) => {
        const rowOrigin = String(row.originAirport || defaultOrigin || "").trim().toUpperCase();
        if (!rowOrigin || !runOriginSet.has(rowOrigin)) return false;
        const originDates = perOriginDates[rowOrigin];
        return !originDates || originDates.has(row.flightDate);
      });
      const fr24Meta = fr24Loaded.meta || {};
      const fr24Metrics = fr24Meta.metrics || {};
      events.emit("fr24_source_selected", {
        source: fr24Meta.source || "unknown",
        rowCount: scopedRows.length,
        latencyMs: fr24LoadLatencyMs,
        fallbackReason: fr24Meta.fallbackReason || null
      });
      console.log(
        `[dashboard] fr24 source=${fr24Meta.source || "unknown"} origins=${runOrigins.join(",")} raw=${fr24Meta.rawRowsCount || 0} normalized=${fr24Meta.normalizedRowsCount || parsedRows.length} filtered=${scopedRows.length} requests=${fr24Metrics.requests || 0} retries=${fr24Metrics.retries || 0}${fr24Meta.fallbackReason ? ` fallback=${fr24Meta.fallbackReason}` : ""}`
      );

      // Schedule projection: for origins whose lookahead exceeds FR24 data,
      // project known destinations forward to fill the uncovered dates.
      const projectedRows = [];
      for (const origin of runOrigins) {
        const originDates = perOriginDates[origin];
        if (!originDates) continue;
        const originRows = scopedRows.filter(
          (r) => String(r.originAirport || "").trim().toUpperCase() === origin
        );
        if (originRows.length === 0) continue;

        // Dates covered by FR24 data for this origin
        const coveredDates = new Set(originRows.map((r) => r.flightDate));
        const missingDates = [...originDates].filter((d) => !coveredDates.has(d));
        if (missingDates.length === 0) continue;

        // Collect unique destinations with representative airline info
        const destMap = new Map();
        for (const row of originRows) {
          if (isCargoOperator(row.airline)) continue;
          const dest = String(row.destinationIata || "").trim().toUpperCase();
          if (!dest || destMap.has(dest)) continue;
          destMap.set(dest, {
            destinationIata: dest,
            destinationCity: row.destinationCity || dest,
            airline: row.airline || "",
            carrierCode: row.carrierCode || ""
          });
        }

        // Create one synthetic row per (origin, destination, missing date)
        for (const date of missingDates) {
          for (const info of destMap.values()) {
            projectedRows.push({
              originAirport: origin,
              flightDate: date,
              destinationIata: info.destinationIata,
              ...
              flight: "",
              timeLocal: "12:00",
              departureLocalIso: `${date}T12:00:00`,
              status: "Projected"
            });
          }
        }
        console.log(
          `[dashboard] projected ${missingDates.length} missing date(s) × ${destMap.size} destination(s) = ${missingDates.length * destMap.size} synthetic rows for ${origin}`
        );
      }

      const allScopedRows = projectedRows.length > 0 ? [...scopedRows, ...projectedRows] : scopedRows;

      const allDashboardRows = allScopedRows
        .filter((row) => !isCargoOperator(row.airline))
        .map((row) => toDashboardRow(row, row.originAirport || defaultOrigin, observedAt));
      ...
      for (const row of allDashboardRows) {
        if (isDepartureTooSoon(row.departureLocalIso)) {
          if (config.dashboardLogSkipDetails) {
            ...
            console.log(`[skip] ${row.origin}→${row.destinationIata} ${flightCode} departed or departing too soon — ${timeLocal} DXB time`);
          }
          skippedCount += 1;
          continue;
        }
      ...
      console.log(
        `[prefilter] skipped ${skippedCount} past/too-soon departures, ${dashboardRows.length} remaining (details=${config.dashboardLogSkipDetails ? "on" : "off"})`
      );
      ...
      try {
        runId = await storage.createRun({
          startedAt,
          status: "running",
          origin: runOrigins.join(","),
          lookaheadDays: runConfig.dashboardLookaheadDays,
          totalTasks: tasks.length,
          completedTasks: 0,
          nextRunAt: nextAt
        });
      } catch (err) {
        if (err?.code === "RUN_ALREADY_IN_PROGRESS") {
          console.warn("[dashboard] run skipped: storage reports another run already in progress");
          return { status: "skipped", reason: "run_in_progress" };
        }
        throw err;
      }
      ...
      currentRun = {
        ...
        completedTasks: 0,
        nextRunAt: nextAt,
        fr24Source: fr24Meta.source || "unknown",
      ...
      const nextAtAfterCompletion = computeNextAlignedRun(config.dashboardIntervalMinutes).iso;
      ...
      await storage.finalizeRun(runId, {
        ...
        nextRunAt: nextAtAfterCompletion,
        error: null
      });

      console.log(`[dashboard] run lifecycle: finalized run=${runId} status=active flights=${finalRows.length}`);
      const fr24Credits = getFr24CreditUsage(config);
      console.log(
        `[fr24] credits: today=${fr24Credits.today} month_estimate=${fr24Credits.monthEstimate} budget=${fr24Credits.budget}`
      );
      if (fr24Credits.shouldWarn) {
        console.warn(
          `[fr24] projected monthly usage ${fr24Credits.monthEstimate} exceeds ${fr24Credits.warnPercent}% of budget ${fr24Credits.budget}`
        );
      }
      ...
      events.emit("run_completed", {
        ...
        nextRunAt,
        fr24Source: fr24Meta.source || "unknown",
        fr24RawRows: fr24Meta.rawRowsCount || parsedRows.length,
        fr24NormalizedRows: fr24Meta.normalizedRowsCount || parsedRows.length,
        fr24Requests: fr24Metrics.requests || 0,
        fr24Retries: fr24Metrics.retries || 0,
        fr24FallbackReason: fr24Meta.fallbackReason || null,
        origins: runOrigins,
        mostRecent: await getMostRecent()
      });
      ...
      try {
        if (currentRun?.id) {
          await storage.finalizeRun(currentRun.id, {
            completedAt,
            status: "failed",
            totalTasks: currentRun.totalTasks || 0,
            completedTasks: currentRun.completedTasks || 0,
            nextRunAt: currentRun.nextRunAt || null,
            error: err.message
          });
          console.log(`[dashboard] run lifecycle: finalized run=${currentRun.id} status=archived (failure: ${err.message})`);
        }
      } catch (finalizeErr) {
        if (currentRun?.id) {
          console.error(`[dashboard] failed to finalize run ${currentRun.id}:`, finalizeErr.message);
        } else {
          console.error("[dashboard] run failed before persistence:", finalizeErr.message);
        }
      }
  ...
  function startScheduler() {
    if (scheduler) return;
    if (isWithinAlignedSlotGrace(config.dashboardIntervalMinutes)) {
      runNow("scheduled_startup_catchup").catch((err) => {
        console.error(`[dashboard-run] ${err.message}`);
      });
    }
